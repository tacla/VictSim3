...
- Lê ../dataset/vict/100v/data.csv
- Gráficos: boxplot+dispersão e médias com IC95%
- Métricas: Eta (correlation ratio), ANOVA (p), estatísticas por grupo
- Saída: ../dataset/vict/100v/avpu_sobr/
"""
...
INPUT = BASE / "data.csv"
OUTDIR = BASE / "avpu_sobr"
ORDER = ["A", "V", "P", "U"]  # ordem desejada para AVPU

def correlation_ratio(categories, values):
    """Eta (0..1): quanto da variação de 'values' é explicada por 'categories'."""
...
def main():
    ...
    # Normaliza AVPU para maiúsculas e aplica ordem
    avpu = df["avpu"].astype("string").str.strip().str.upper()
    sobr = pd.to_numeric(df["sobr"], errors="coerce")
    df2 = pd.DataFrame({"avpu": avpu, "sobr": sobr})
    df2 = df2.dropna(subset=["sobr"])

    # Mantém também contagem de valores inesperados e ausentes
    total_rows = len(df2)
    unexpected = sorted(set(df2["avpu"].dropna()) - set(ORDER))
    na_count = int(df["avpu"].isna().sum())

    # Filtra apenas níveis esperados para análise primária (mantendo relatório dos inesperados)
    ...
    # Eta e ANOVA (apenas níveis presentes)
    ...
    plt.tight_layout()
    plt.savefig(OUTDIR / "avpu_sobr_box_scatter.png", dpi=150)
    plt.close()
    ...
    # quando N<2 não há IC; trate como zero para não quebrar o plot
    ...
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.savefig(OUTDIR / "avpu_sobr_means_ci.png", dpi=150)
    plt.close()
    ...
    lines.append("\n## Gráficos gerados")
    lines.append("- `avpu_sobr_box_scatter.png`")
    lines.append("- `avpu_sobr_means_ci.png`\n")
